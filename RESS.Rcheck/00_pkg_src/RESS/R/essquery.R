essQuery <- function(essentia, aq = "", flags = "") {
    ...
    if (((substr(essentia, 1, 8) == "ess exec") && (!grepl("#Rignore", flags))) || ((substr(essentia, 1, 10) == "ess stream" || substr(essentia, 1, 9) == "ess query") && (grepl("#Rinclude", flags)))) {
      ...
      remove(t2)
      if ((file > 1) && (separate)) {
        print(sprintf("---------------- Stream Completed: %i files stored in %i commands: %s%i to %s%i ----------------", file, file, varname, commandcount - file, varname, commandcount - 1))
	if (grepl("#filelist",line)) {
        	...
        	assign(sprintf("%s%i", varname, commandcount), t2[1:file,1:ncol(t2)], inherits = TRUE)
	        print(get(sprintf("%s%i", varname, commandcount)))
        	print(sprintf("---------------- Filenames stored in %s%i ----------------", varname, commandcount))
	        commandcount <- commandcount + 1
        	close(t1)
	        remove(t1)
        	remove(t2)
